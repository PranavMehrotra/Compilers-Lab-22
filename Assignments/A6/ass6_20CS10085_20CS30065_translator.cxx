...
bool typecheck(ST_entry* &s1, ST_entry* &s2) {
    ...
    if(typecheck(t1, t2))
        return true;
    else if(t1->type=="float" or t1->type=="float") {
        s1 = convert_type(s1,"float");
        s2 = convert_type(s2,"float");
        return true;
    }
    else if(t1->type=="int" or t1->type=="int") {
        s1 = convert_type(s1,"int");
        s2 = convert_type(s2,"int");
        return true;
    }
